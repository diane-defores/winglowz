...
linked_systems:
  ...
  - "Convex"
evidence:
  - "User question 2026-05-23: how to handle NoteFlowz/other products and French apps that may not naturally belong to the WinFlowz/Wispr Flow suite."
  ...
  - "/home/claude/shipflow_data/CLAUDE.md lists NoteFlowz, plaisirsurprise, French content projects, Convex and Clerk patterns."
  - "/home/claude/shipflow_data/PROJECTS.md lists French or separate-brand products including jarrettelacoke.fr and plaisirsurprise."
  - "/home/claude/shipflow_data/projects/winflowz/docs/technical/suite-authentication.md defines Clerk central identity plus per-product entitlements for the WinFlowz suite."
depends_on:
...
How should products such as NoteFlowz/NoteFinderz and French apps fit with the new shared identity model, especially when they may not feel like obvious WinFlowz suite products to users?

## Context Read

- `docs/explorations/2026-05-17-root-apps-firebase-fit.md` - existing portfolio classification across WinFlowz, ReplayGlowz, ContentGlowz, NoteFinderz, Nantes Gratuit and Quit Coke.
- `/home/claude/shipflow_data/CLAUDE.md` - workspace-level stack notes for NoteFlowz, French projects, Convex and Clerk.
- `/home/claude/shipflow_data/PROJECTS.md` - high-level list of separate products and brands.
- `/home/claude/shipflow_data/projects/winflowz/docs/technical/suite-authentication.md` - canonical WinFlowz suite auth decision.
...
### Option A: One Visible WinFlowz Account For Everything

- Summary: every product uses the same visible WinFlowz/Flowz account brand.
- Pros: simplest support story, strongest cross-sell, fewer provider dashboards.
- Cons: confusing or damaging for unrelated products. A Nantes Gratuit, quit-coke, health, luxury or local-service user may not expect a WinFlowz consent screen.
...
- Productivity / AI workflow products: WinFlowz, ReplayGlowz, ContentGlowz, NoteFlowz/NoteFinderz and possibly SocialGlowz can share a Flowz/WinFlowz family identity if login copy clearly says the product uses the shared account.
- Local French / public-interest products: Nantes Gratuit should not show a WinFlowz account brand unless the product is explicitly repositioned under that umbrella.
...
- Exact final public umbrella name: WinFlowz, Flowz, Diane Apps, or something else remains open.
...
- Presenting every Google consent screen as WinFlowz by default - rejected because it can confuse users of French/local/sensitive apps.
