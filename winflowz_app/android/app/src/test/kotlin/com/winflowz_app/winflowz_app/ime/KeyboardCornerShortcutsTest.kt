...
class KeyboardCornerShortcutsTest {
    ...
    fun `smart french corners also resolve on azerty layout`() {
        ...
        assertEquals("W←", eKey.cornerAssignments.bottomLeft?.label)
        assertEquals("W→", eKey.cornerAssignments.bottomRight?.label)
        ...
        assertEquals(KeyboardKeyAction.NavigateWordLeft, eKey.cornerAssignments.bottomLeft?.value?.action)
        assertEquals(KeyboardKeyAction.NavigateWordRight, eKey.cornerAssignments.bottomRight?.value?.action)
    ...
    @Test
    fun `azerty e d u and j directional gestures ignore special-key toggle`() {
