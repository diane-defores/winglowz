...
class WinFlowzKeyboardView(
    ...
    private val ctrlHoldForceAddedPointerIds = linkedSetOf<Int>()
    ...
    private fun drawLongPressSwipeVisuals(canvas: Canvas): Boolean {
        ...
        for ((pointerId, visual) in activePointers) {
            ...
            val startFrame = keyFrameForId(visual.startKeyId)
            if (startFrame == null) {
                toClear.add(pointerId)
                continue
            }
            drawLongPressSwipeVisual(
                canvas = canvas,
                startX = startFrame.visualRect.centerX(),
                startY = startFrame.visualRect.centerY(),
                endX = state.latestX,
                endY = state.latestY,
    ...
    private fun drawSwipeTrackGuide(
        ...
        val guidePath = buildSwipeGuidePath(
            startX = startX,
            startY = startY,
            endX = endX,
            endY = endY,
            phaseOffset = (startX * 0.013f + startY * 0.017f + endX * 0.019f + endY * 0.023f) % (2f * kotlin.math.PI.toFloat()),
        )
        ...
        canvas.drawPath(guidePath, longPressSwipeStrokePaint)
    ...
    private fun buildSwipeGuidePath(
        startX: Float,
        startY: Float,
        endX: Float,
        endY: Float,
        ...
        keyEffectPath.moveTo(startX, startY)
        val segments = 20
        val livePhase = (SystemClock.uptimeMillis() % 850L) / 850f * kotlin.math.PI.toFloat() * 2f
        for (index in 1..segments) {
            val t = index / segments.toFloat()
            val x = startX + dx * t
            val y = startY + dy * t
            val weight = (1f - kotlin.math.abs(0.5f - t) * 2f).coerceAtLeast(0f)
            val wave = sin((phaseOffset + livePhase * 0.3f + t * 18f).toDouble()).toFloat() *
                dp(2.4f * longPressSwipeGuidelineScale) * (0.3f + 0.7f * weight)
    ...
    private fun drawSwipeSpecularPath(
        ...
        drawSwipeArcPath(
            ...
            normalX,
            normalY,
            KeyboardPressEffectSpec("specularSweep", 1, intensity, "linear"),
            accentColor,
            intensity,
            progress * 18.849556f,
            wobble = false,
    ...
    private fun keyFrameForId(keyId: String): KeyFrame? {
        return keyFrames.firstOrNull { it.key.id == keyId }
    }
    ...
    private fun isLongPressSwipeHoverEligible(key: KeyboardKeySpec): Boolean {
        return key.enabled &&
            ...
            (key.cornerAssignments.up != null ||
                key.cornerAssignments.right != null ||
                key.cornerAssignments.down != null ||
                key.cornerAssignments.left != null)
    }
    ...
    private fun activateActionLongPressSwipeMode(
        ...
        longPressSwipeActionRowsBeforeByPointerId[pointerId] =
            actionBarState.attachedRows
                ...
                .map { it.rowId }
                .toSet()
        registerLongPressSwipeVisual(pointerId, startKeyId)
    ...
    private fun cleanupPointerStateAfterRemoval(pointerId: Int) {
        ...
        if (ctrlHoldPointerIds.remove(pointerId)) {
            ctrlHoldForceAddedPointerIds.remove(pointerId)
            if (ctrlHoldForceAddedPointerIds.isEmpty()) {
    ...
    private fun drawKey(
        ...
        if (!hideInactiveLongPressSwipeSurface) {
            ...
            if (reliefDepth > 0f) {
                drawKeyRelief(canvas, drawRect, drawRadius, paint.color, pressed)
            }
        ...
        canvas.drawText(displayLabelForRect(key, drawRect), drawRect.centerX(), baseline, textPaint)
    ...
    private fun drawKeyRelief(
        ...
        val depth = dp(themeConfig.keyReliefDepth).coerceIn(0f, dp(6f))
        if (depth <= 0f) return
        keyReliefDarkPaint.strokeWidth = max(dp(1f), depth)
        keyReliefLightPaint.strokeWidth = max(dp(0.75f), depth * 0.55f)
        keyReliefDarkPaint.color = keyboardLayerColor(adjustColor(baseColor, 0.72f), KEYBOARD_BORDER_OPACITY_BOOST)
        keyReliefLightPaint.color = keyboardLayerColor(adjustColor(baseColor, 1.18f), KEYBOARD_BORDER_OPACITY_BOOST)
        ...
        canvas.clipRect(
            rect.left,
            rect.centerY() - if (pressed) depth * 0.15f else 0f,
            rect.right,
            rect.bottom + depth,
        )
        ...
        if (!pressed) {
            ...
            canvas.clipRect(rect.left, rect.top - depth, rect.right, rect.centerY())
