...
CloudSyncCategoryStatus _suiteAccessStatus({
  ...
  if (!remoteAuthConfigured) {
    return const CloudSyncCategoryStatus(
      ...
      title: 'Accès WinFlowz',
      state: CloudSyncCategoryState.unavailable,
      stateLabel: 'Non configuré',
      detail: 'Le cloud WinFlowz n’est pas disponible dans cette version.',
    );
  }
  if (authError != null) {
    return const CloudSyncCategoryStatus(
      ...
      detail: 'Le statut d’accès WinFlowz est indisponible pour l’instant.',
    );
  }
  if (authLoading || authSession == null || suiteLoading) {
    return const CloudSyncCategoryStatus(
      ...
      state: CloudSyncCategoryState.checking,
      stateLabel: 'Vérification',
      detail: 'L’accès WinFlowz est vérifié.',
    );
  }
  if (!authSession.isSignedIn || authSession.isLocalFallback) {
    return const CloudSyncCategoryStatus(
      ...
      stateLabel: 'Non connecté',
      detail:
          'Connecte ton compte WinFlowz pour activer la synchronisation de données.',
    );
  }
  if (suiteError != null) {
    return const CloudSyncCategoryStatus(
  ...
  final hasAccess =
      suiteIdentity?.statusFor(ProductId.winflowzApp) ==
  ...
  if (hasAccess) {
    return const CloudSyncCategoryStatus(
      ...
      stateLabel: 'Active',
      detail:
          'Le compte a bien accès WinFlowz. La synchronisation peut être active '
...
CloudSyncCategoryStatus _dataCategoryStatus({
  ...
  if (!hasSuiteAccess) {
    return CloudSyncCategoryStatus(
      ...
      title: title,
      state: CloudSyncCategoryState.localOnly,
      stateLabel: 'Accès WinFlowz inactif',
      detail:
          'Le compte existe mais pas d’accès WinFlowz actif pour cette catégorie.',
    );
  }
  if (!remoteEnabled) {
    return CloudSyncCategoryStatus(
...
CloudSyncCategoryStatus _keyboardStatus({
  ...
  if (!remoteAuthConfigured ||
    ...
    return const CloudSyncCategoryStatus(
      ...
      detail:
          'Le profil clavier Android dépend d’un compte cloud WinFlowz actif.',
    );
  }
  if (suiteIdentity?.statusFor(ProductId.winflowzApp) !=
      SuiteAccountStatus.accessActive) {
    return const CloudSyncCategoryStatus(
      category: CloudSyncCategory.keyboardProfile,
      title: 'Profil clavier Android',
      ...
      detail: 'Sans accès WinFlowz actif, le profil clavier reste local.',
    );
  }
