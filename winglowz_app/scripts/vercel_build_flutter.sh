...
add_dart_define \
  ...
  "${WINGLOWZ_APP_BUILD_SHA:-${VERCEL_GIT_COMMIT_SHA:-local}}"
add_dart_define \
  WINGLOWZ_APP_BUILD_RUN_ID \
  "${WINGLOWZ_APP_BUILD_RUN_ID:-${VERCEL_DEPLOYMENT_ID:-${VERCEL_GIT_COMMIT_SHA:-local}}}"
add_dart_define \
  WINGLOWZ_APP_BUILD_REF \
  "${WINGLOWZ_APP_BUILD_REF:-${VERCEL_GIT_COMMIT_REF:-local}}"
add_dart_define \
  WINGLOWZ_APP_BUILD_AT_UTC \
  "${WINGLOWZ_APP_BUILD_AT_UTC:-$(date -u +%Y-%m-%dT%H:%M:%SZ)}"
add_dart_define \
  WINGLOWZ_APP_BUILD_AT_PARIS \
  "${WINGLOWZ_APP_BUILD_AT_PARIS:-$(TZ=Europe/Paris date '+%Y-%m-%d %H:%M')}"
