...
supersedes: []
evidence:
  - "Canonical suite auth decision documented in the main WinGlowz project on 2026-05-17."
  ...
  - "Firestore entitlement enforcement added on 2026-05-21: Firestore rules require a server-owned `suiteAccess/{uid}` mirror for `winglowz_app`, and product stores stay local unless the suite identity snapshot grants access."
...
- Firestore product data under `users/{uid}` is gated by a server-owned `suiteAccess/{uid}` mirror. The app also refuses to select Firestore-backed product stores unless `suiteIdentityProvider` grants `winglowz_app`.
