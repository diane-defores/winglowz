...
evidence:
  ...
  - "Updated for materialized IME press effects with relief-aware rendering."
...
directly: overlay foreground service, accessibility fallback, and the WinGlowz
...
| `android/app/src/main/kotlin/com/winglowz_app/winglowz_app/MainActivity.kt` | Flutter MethodChannel owner | Keep `winglowz_app/overlay` and `winglowz_app/keyboard` contracts separate. |
| `android/app/src/main/kotlin/com/winglowz_app/winglowz_app/Overlay*.kt` | Overlay runtime | Do not allow concurrent overlay and keyboard recording without coordinator logic. |
| `android/app/src/main/kotlin/com/winglowz_app/winglowz_app/ime/**` | Native IME services/controllers | No raw text logging; sensitive fields disable capture/sync/enriched insertion by default; an explicit advanced opt-in may allow clipboard history only. Clipboard bridge events persist only as a bounded local drain queue for Flutter import. |
| `android/app/src/main/res/xml/**` | Android service metadata | IME metadata must match manifest service declarations. |

## Entrypoints

- `WinGlowzInputMethodService.onCreateInputView`: creates the native keyboard.
- `WinGlowzInputMethodService.onStartInput`: evaluates the focused field policy and field context (`text`, `email`, `url`, `phone`, `search`).
...
```text
Android input field
  -> WinGlowzInputMethodService
  -> KeyboardInputContextResolver + KeyboardSecurityPolicy
  -> WinGlowzKeyboardView (Canvas)
...
Keyboard clipboard action
  ...
  -> winglowz_app/keyboard MethodChannel drain
...
- Keyboard preferences are persisted in `KeyboardStateStore` and round-tripped through `winglowz_app/keyboard`:
...
- Keyboard theme authoring lives in Flutter `KeyboardThemeStudioScreen` and is pushed to native with `getKeyboardThemeConfig`, `setKeyboardThemeConfig`, and `resetKeyboardThemeConfig` on `winglowz_app/keyboard`. The studio ships the v1 preset catalog (`System`, `WinGlowz Light`, `WinGlowz Dark`, `Neon Terminal`, `Glass Mint`, `Sunset Gradient`, `Midnight Aurora`, `Paper Ink`, `Pixel Candy`, `Minimal Contrast`), collapsible editing sections, and JSON import/export without image bytes.
- Native IME `onThemeSettings()` opens Flutter route `/keyboard/theme` through `MainActivity` intent extra `openRoute`.
- Theme saves are blocked in Flutter when key/status contrast falls below the readable threshold or when image mode has no imported local image. Heavy shadows, thick borders, high effect intensity, long particle effects, and long mascot trails produce warnings.
- Native rendering supports solid, linear gradient, radial gradient, app-private image backgrounds, key border/radius/shadow values, custom key/status/corner text colors, relief key faces, and bounded press effects. Private fields suppress custom image/gradient/effect surfaces.
- Native press effects are split by rendering contract. `scale`, `pulse`, `shake`, `glow`, `electricArc`, `specularSweep`, `inkPress`, `keycapTilt`, and `edgeCompression` must render through the key surface geometry in `WinGlowzKeyboardView`; they must move or tint the physical key body instead of drawing an independent overlay. `ripple`, `confettiLite`, `fireworksLite`, `waterSplash`, and `emberBurst` may use `KeyboardPressEffects`, but only as queue-capped emissions clipped or anchored to the pressed key. `dragonTrail` and `spiderTrail` use `KeyboardPressEffects` as bounded mascot effects with position, latest target, retargeting, and a short fading trail: dragon adds fading ember sparkles, while spider adds fine web-like cross strands. Mascots draw a small shadow and can depress relief-enabled keys with a partial hover/weight cue; this visual pressure must not change input dispatch, active key state, or pressed-key color. Effects are disabled for private fields or system animation scale `0`.
- Keyboard diagnostics expose `themePressEffectRenderingMode` and `themeKeyReliefDepth` with no typed content, clipboard content, snippets, dictation content, tokens, prompts, or private image paths.
...
- `WinGlowzKeyboardView` wraps draw/touch/dispatch/layout refresh paths. A recoverable exception stops repeat runnables, clears the active gesture, stores the diagnostic through `KeyboardCrashReporter`, and switches to a neutral `KeyboardLayoutBuilder.safeFallback()` snapshot without deleting user preferences.
- `WinGlowzInputMethodService` wraps lifecycle preference refresh and system actions such as settings/theme launch and keyboard picker. Service-level failures are reported through the same redigé diagnostic store and shown as `Keyboard recovered`.
- Flutter Settings can call `getKeyboardStatus` and `clearKeyboardDiagnostics` on `winglowz_app/keyboard`. `Clear logs` clears both `AppDiagnostics` and native keyboard diagnostics; `Copy diagnostic` includes the last native incident when present.
...
2. Enable/switch to WinGlowz in a real Android text field.
...
- Manifest or `res/xml` changed -> verify WinGlowz appears in Android keyboard settings.
...
- `KeyboardCornerShortcuts.kt`, `KeyboardLayoutModels.kt`, `WinGlowzKeyboardView.kt`, or `KeyboardStateStore.kt` changed -> recheck Smart French defaults, override precedence, private-field suppression, special-key toggle, directional/corner rendering, space slider priority, scroll row priority, and corrupt JSON fallback.
- `KeyboardPressEffects.kt`, `WinGlowzKeyboardView.kt` material effect geometry, or theme validation changed -> recheck fast typing, private/password fields, relief mode, system reduce-motion, and unreadable theme rejection.
