...
import com.winglowz_app.winglowz_app.WinGlowzNotificationListenerService
...
import com.winglowz_app.winglowz_app.ime.actions.KeyboardActionRowSpec
...
class KeyboardStateStore(private val context: Context) {
    ...
    fun themeConfig(): KeyboardThemeConfig {
        ...
        return when (themeMode) {
            THEME_LIGHT -> parsed.copy(presetId = "winglowz_light")
            THEME_DARK -> parsed.copy(
                presetId = "winglowz_dark",
    ...
    private fun isInputMethodEnabled(): Boolean {
        ...
        return manager.enabledInputMethodList.any(::isWinGlowzIme)
    }
    ...
    private fun isInputMethodActive(): Boolean {
        ...
        return current.contains(context.packageName) &&
            current.contains(WinGlowzInputMethodService::class.java.simpleName)
    }

    fun isMediaSessionAccessGranted(): Boolean {
        val expected = ComponentName(context, WinGlowzNotificationListenerService::class.java).flattenToString()
        val enabled =
            Settings.Secure.getString(
                context.contentResolver,
    ...
    private fun isWinGlowzIme(info: InputMethodInfo): Boolean {
        val serviceName = info.serviceName ?: return false
        return info.packageName == context.packageName &&
            serviceName.endsWith(WinGlowzInputMethodService::class.java.simpleName)
    }
    ...
    companion object {
        const val PREFERENCES_NAME = "winglowz_app_keyboard_prefs"
