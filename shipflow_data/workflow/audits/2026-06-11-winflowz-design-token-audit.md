...
# WinFlowz Design Tokens Audit

## Verdict

WinFlowz has a usable token base on both app and site, but migration is not yet
...
- `winflowz_site/src/layouts/LandingLayout.astro:55` -> hardcoded `#ff00c8` for `meta theme-color`.
- `winflowz_site/src/pages/manifest.json.ts:55-56` -> hardcoded `#000000` / `#ffffff`.
- `winflowz_site/src/components/Button.astro` -> multiple spacing/typography/color literals in component CSS.
- `winflowz_site/src/pages/[...lang]/bio.astro` -> repeated hardcoded dimensions, typography, transition and color literals.
- `winflowz_app/lib/features/keyboard/presentation/keyboard_theme_studio_screen.dart` -> direct keyboard color constants and style values outside token files.
- `winflowz_app/lib/core/theme/app_theme.dart` -> only one file containing many theme-level values, while UI files still embed additional literals in the site branch.

## Top files to clean first

1. `winflowz_site/src/assets/styles/global.css` (`54` findings)
2. `winflowz_site/src/assets/styles/docs.css` (`52` findings)
3. `winflowz_site/src/components/Button.astro` (`13+` findings)
4. `winflowz_site/src/pages/[...lang]/bio.astro` (`31` findings)
5. `winflowz_app/lib/features/keyboard/presentation/keyboard_theme_studio_screen.dart` (`Color(0x...)` usages outside token files)

## Remediation order (recommended)

1. **Token authority lock**
   - Finalize and enforce `shipflow_data/technical/design-system-authority.md` as the
     mandatory source.
2. **App scan reduction**
   - Move remaining site/app hard-coded values in `keyboard_theme_studio_screen.dart` into
     dedicated token abstractions and consume through `WinFlowzThemeTokens`.
...
Titre proposé: WinFlowz token hardening and visual standardization
...
Recommended route: `/100-sf-spec WinFlowz token hardening and visual standardization`
