...
owner: "Diane"
confidence: medium
user_story: "En tant qu'utilisatrice WinFlowz, je veux une cohérence visuelle stricte entre app, site et composants, sans déviation locale de tokens, afin que l'identité produit soit pro, lisible et maintenable sur mobile et desktop."
risk_level: "high"
...
linked_systems:
  ...
  - "winflowz_app/lib/features/keyboard/presentation/keyboard_theme_studio_screen.dart"
  - "winflowz_site/src/components/Button.astro"
depends_on:
...
supersedes: []
evidence:
  - "python3 /home/claude/shipflow/tools/design_system_drift_check.py --format markdown --warn-only --max-findings 5000"
  - "Excluding generated .vercel/output artifacts: 422 findings still actionable"
  - "Canonique visuel recommandé dans `shipflow_data/technical/design-system-authority.md`"
next_step: "/102-sf-start WinFlowz token hardening and visual standardization"
---

# Title

WinFlowz Token Hardening and Visual Standardization
...
En tant qu'utilisatrice WinFlowz, je veux une cohérence visuelle stricte sur toutes les surfaces (application Android/web, site de présentation, composants partagés), afin d'avoir une expérience professionnelle stable, cohérente, et sans régression de branding, sur mobile et desktop.

## Minimal Behavior Contract

WinFlowz enforces that every production visual decision about color, spacing, typography, motion, radius, elevation, layout sizing, and theme-sensitive branching flows from the canonical token sources per project surface; non-authoritative visual literals are rejected unless explicitly whitelisted as temporary, documented platform-bound exceptions. When a screen or component is rendered, its visual output must be deterministic from token sources and shared component abstractions, and if a visual override is blocked at runtime or by migration, the app/site continues in a safe, branded fallback style rather than rendering ad-hoc constants. The main edge case is cross-surface drift (app/site/integration) where one surface compiles with updated tokens and another silently keeps legacy values.

## Success Behavior

- Given production Flutter UI code and site components are modified under this spec, when `design_system_drift_check.py --changed` runs on changed files, then no unapproved hardcoded visual literals are reported in `winflowz_app/lib` and `winflowz_site/src`, excluding `winflowz_site/.vercel/output/**`.
- Given a developer applies or updates a visual token, when they touch shared components, then shared tokens in `winflowz_theme_tokens.dart` or `global.css`/`tailwind.config.mjs` are updated first and consumed by the target component without adding bypass values.
...
- Application Flutter: suppression des usages visuels hardcodés non autorisés dans `winflowz_app/lib`.
- Site Astro: remplacement des utilitaires Tailwind visuels arbitraires et des hex/rgb dans `winflowz_site/src`.
- Application des règles de design-system à:
  - `winflowz_site/src/assets/styles/global.css`
  - `winflowz_site/tailwind.config.mjs`
  - `winflowz_app/lib/core/theme/winflowz_theme_tokens.dart`
  - `winflowz_app/lib/core/theme/app_theme.dart`
  - composants partagés (notamment `winflowz_app/lib/core/widgets/app_components.dart`, `winflowz_site/src/components/Button.astro`)
- Validation automatisée par drift-check ciblé sur fichiers changés, plus vérifications locales de build/lint.
- Documentation de chantier dans `shipflow_data/technical/design-system-authority.md` et `shipflow_data/technical/guidelines.md` si nécessaire.
...
- `winflowz_app/lib/features/keyboard/presentation/keyboard_theme_studio_screen.dart` peut rester en exception temporaire uniquement si documenté avec TTL et plan de migration.
- `winflowz_site/.vercel/output/**` est exclu du scope de preuve.
...
- Contrat technique visuel canonique : `shipflow_data/technical/design-system-authority.md`.
- Contrat produit/branding : `shipflow_data/business/branding.md`.
- Contraintes techniques : `shipflow_data/technical/guidelines.md`.
- Contexte Flutter App : `shipflow_data/technical/winflowz_app/guidelines.md`.
- Contrat d’architecture : `shipflow_data/technical/architecture.md`.
- Outil de contrôle qualité : `/home/claude/shipflow/tools/design_system_drift_check.py`.
- Script de validation local Flutter : `flutter analyze`, `flutter test` dans `winflowz_app` si mutation Flutter.
- Script de validation site : `pnpm build:check` dans `winflowz_site` si mutation site.
...
- checklist_path: `shipflow_data/workflow/test-checklists/winflowz-token-hardening-and-visual-standardization.md`.
...
- Le branding WinFlowz reste cohérent entre app et site dans l'espace visuel canonique.
- Les composants UI applicatifs doivent lire les styles via `WinFlowzThemeTokens`/tokens CSS/aliases.
...
- `winflowz_app/lib` : modifications à `app_theme.dart`, `winflowz_theme_tokens.dart`, composants UI et éventuellement écrans Flutter touchés.
- `winflowz_site/src` : modifications aux styles globaux, composants UI, et pages en production.
- `shipflow_data/technical/design-system-authority.md` : devient le garde-fou opérationnel de design-system.
- `shipflow_data/workflow/TASKS.md` : tâches résiduelles de dette visuelle si des exceptions temporaires restent ouvertes.
- Documentation utilisateur et brand pages may need alignment if copy around theming/help text changes.

## Documentation Coherence

- Mettre à jour, si besoin, `shipflow_data/technical/design-system-authority.md` avec la liste finale des exceptions acceptées.
- Mettre à jour `shipflow_data/technical/guidelines.md` si de nouvelles règles de migration ou de guardrails sont promulguées.
...
- [ ] Tâche 1: Verrouiller la doctrine d’autorité visuelle
  - Fichiers: `shipflow_data/technical/design-system-authority.md`, `shipflow_data/technical/guidelines.md`
...
- [ ] Tâche 2: Introduire les tokens manquants de tokens partagés (app)
  - Fichiers: `winflowz_app/lib/core/theme/winflowz_theme_tokens.dart`, `winflowz_app/lib/core/theme/app_theme.dart`
...
- [ ] Tâche 3: Régulariser `keyboard_theme_studio_screen.dart` vers tokens
  - Fichiers: `winflowz_app/lib/features/keyboard/presentation/keyboard_theme_studio_screen.dart`
...
- [ ] Tâche 4: Réduire les valeurs littérales dans composants Flutter partagés
  - Fichiers: composants UI principaux dans `winflowz_app/lib/core/widgets`, `winflowz_app/lib/features/**`
  ...
  - Depends on: Tâche 2.
  - Validate with: `python3 /home/claude/shipflow/tools/design_system_drift_check.py --changed --format markdown`.

- [ ] Tâche 5: Recentrer le système de composants UI du site
  - Fichiers: `winflowz_site/src/assets/styles/global.css`, `winflowz_site/tailwind.config.mjs`
...
- [ ] Tâche 6: Nettoyer `Button.astro`
  - Fichier: `winflowz_site/src/components/Button.astro`
...
- [ ] Tâche 7: Nettoyer les landing/components critiques
  - Fichiers: `winflowz_site/src/pages/[...lang]/bio.astro`, `winflowz_site/src/components/sections/landing/HeroSectionAlt.astro`, `winflowz_site/src/assets/styles/docs.css`, `winflowz_site/src/assets/styles/landing.css`, `winflowz_site/src/components/shared/site/Navbar.astro`
...
- [ ] Tâche 8: Mettre en place le contrôle de preuve visuelle
  - Fichiers: `shipflow_data/workflow/test-checklists/winflowz-token-hardening-and-visual-standardization.md`, `shipflow_data/workflow/audits/` si nécessaire
...
- [ ] Tâche 9: Re-lancer l’audit de drift avant fermeture
  - Fichiers: `shipflow_data/workflow/audits/2026-06-11-winflowz-design-token-audit.md` + nouveaux outputs de drift.
  - Action: obtenir un nouveau baseline, viser une chute significative des findings non-justifiés.
  - Depends on: Tâches 2-8.
  - Validate with: `python3 /home/claude/shipflow/tools/design_system_drift_check.py --format markdown --warn-only --max-findings 5000` et comparaison des KPI.

- [ ] Tâche 10: Finaliser la doc de sortie chantier
  - Fichiers: cette spec, `shipflow_data/technical/design-system-authority.md`
...
- [ ] CA-2: `winflowz_app` and `winflowz_site` production visuals no longer use `Color(0x...)`, `[#A-Za-z0-9]{6}`, `max-w-[...]`, `min-h-[...]`, arbitrary px/rem values unless explicitly documented as exception.
- [ ] CA-3: `winflowz_site/src/assets/styles/global.css` and `winflowz_app/lib/core/theme/*` are the primary anchors for spacing/typography/color/motion tokens.
...
- [ ] CA-5: At least one visual smoke check is executed on a key mobile path (`winflowz_site` landing + one key Flutter screen) after migration.
- [ ] CA-6: `shipflow_data/technical/design-system-authority.md` reflects the enforcement rules and allowed exception list.
...
- Lire d’abord: `shipflow_data/technical/design-system-authority.md`, `shipflow_data/workflow/audits/2026-06-11-winflowz-design-token-audit.md`, `winflowz_app/lib/core/theme/winflowz_theme_tokens.dart`, `winflowz_site/src/assets/styles/global.css`.
