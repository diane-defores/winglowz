...
depends_on:
  - artifact: "shipflow_data/workflow/specs/asr-language-pack-catalog.md"
    artifact_version: "0.1.0"
...
En tant qu'utilisatrice WinFlowz, je veux que la dictée clavier utilise vraiment un moteur ASR local quand un pack est installé et valide, pour pouvoir dicter sans appel serveur implicite et sans interruption silencieuse.

Acteur principal: utilisatrice Android WinFlowz utilisant le clavier.
Acteurs secondaires: produit, support, infrastructure QA.

# Minimal Behavior Contract

Le runtime doit charger et exécuter un moteur local réel à partir d’un chemin de modèle déjà installé dans le store WinFlowz, produire la transcription en local, puis revenir de manière explicite au fallback système si le chargement, l’initialisation ou la reconnaissance locale échouent.
...
Créer une couche d’abstraction minimale dans `android/app/src/main/kotlin/com/winflowz_app/winflowz_app/ime`:
...
- Mettre à jour `shipflow_data/workflow/specs/asr-language-pack-catalog.md`:
