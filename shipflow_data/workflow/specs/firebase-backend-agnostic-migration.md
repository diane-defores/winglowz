...
owner: "Diane"
confidence: high
user_story: "En tant que builder de WinFlowz, je veux remplacer la cible Supabase par des contrats backend-agnostiques avec Firebase comme premier adaptateur Android, afin de garder l'app gratuite au départ, pilotable en CLI et remplaçable plus tard."
risk_level: "high"
security_impact: "yes"
docs_impact: "yes"
linked_systems:
...
En tant que builder de WinFlowz, je veux remplacer la cible Supabase par des contrats backend-agnostiques avec Firebase comme premier adaptateur Android, afin de garder l'app gratuite au départ, pilotable en CLI et remplaçable plus tard.
...
- Dev-only Firebase project ID: `winflowz-dev`. Display name may be `WinFlowz Dev`; Google Cloud project IDs cannot contain underscores.
...
- [x] Tâche 2 : Ajouter le socle Firebase CLI
  ...
  - Validate with : JSON lint OK, `git diff --check` OK; Firebase CLI 15.17.0 installé; `firebase emulators:exec --project demo-winflowz-dev --only firestore,auth "true"` OK.
