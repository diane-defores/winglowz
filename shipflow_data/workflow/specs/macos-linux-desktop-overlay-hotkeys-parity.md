...
depends_on:
  - artifact: "shipflow_data/workflow/specs/windows-desktop-overlay-hotkeys-parity.md"
    artifact_version: "1.0.0"
...
WinFlowz doit conserver le meme modele mental desktop que Windows: une surface
...
- Given WinFlowz est lance sur macOS, when l'utilisateur utilise
  Control+Option+Space, then l'overlay peut s'afficher en fenetre flottante et
  exposer les actions produit communes.
- Given WinFlowz est lance sur Linux, when l'utilisateur appelle l'action
...
- Given un texte final est pret, when la livraison automatique est possible,
  then WinFlowz tente de le coller dans l'app active et garde toujours une copie
  clipboard.
...
- macOS native MethodChannel `winflowz_app/macos_overlay`.
- Linux native MethodChannel `winflowz_app/linux_overlay`.
...
- macOS: `MainFlutterWindow.swift` enregistre `winflowz_app/macos_overlay`,
