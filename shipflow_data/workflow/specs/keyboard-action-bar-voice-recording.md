...
WinFlowz should make the Android keyboard the primary day-to-day interface for voice capture. The floating overlay remains available for users who want it, but it is no longer required for the core voice workflow.
...
- Primary interface: WinFlowz keyboard.
...
- The default path should consume device CPU/NPU/RAM, not WinFlowz server spend.
- Server/worker transcription is a fallback, not the baseline product loop.

## Engine Strategy

- Default: local WinFlowz ASR engine when a supported model is installed and the device passes a lightweight capability check.
...
1. User enables and selects the WinFlowz keyboard.
...
- Extend `WinFlowzInputMethodService` action bar UI with a voice control.
...
- A user can dictate into a normal text field from the WinFlowz keyboard without enabling the overlay.
- A user can dictate without using a WinFlowz worker when a supported local model is installed.
