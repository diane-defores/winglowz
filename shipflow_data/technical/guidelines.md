...
- If content-schema behavior changes, update `shipflow_data/editorial/astro-content-schema-policy.md`.
...
- Update canonical docs in `shipflow_data/technical/` when architecture or product rules change.
- Update canonical docs in `shipflow_data/editorial/` when public claims, content surfaces, or content schemas change.
...
- Canonical governance artifacts live under `shipflow_data/`.
...
```bash
pnpm build:check
python3 /home/claude/shipflow/tools/shipflow_metadata_lint.py shipflow_data/technical/guidelines.md
```
