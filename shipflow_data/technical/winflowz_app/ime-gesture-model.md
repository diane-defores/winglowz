...
depends_on:
  - "shipflow_data/technical/winflowz_app/context.md@1.0.0"
  - "shipflow_data/technical/winflowz_app/context-function-tree.md@1.0.0"
  - "shipflow_data/technical/winflowz_app/code-docs-map.md@0.1.0"
  - "winflowz_app/docs/technical/android-native.md@0.1.0"
supersedes: []
evidence:
  - "WinFlowz IME swipe investigation across WinFlowzKeyboardView.kt and KeyboardLongPressSwipePolicy.kt."
  - "Spec shipflow-code-navigation-and-function-documentation-system.md task 8 pilot."
next_review: "2026-08-04"
next_step: "/300-sf-docs technical audit"
---

# Technical Behavior Index: WinFlowz IME Gestures

## Purpose

This file owns term-based recovery for WinFlowz IME gesture behavior. It exists because operator terms such as `swipe` are ambiguous in the IME and otherwise force repeated search across native code, docs, tests, and workflow artifacts.

## Canonical Role

- `context.md` stays the system overview for WinFlowz technical surfaces.
...
| --- | --- | --- | --- |
| `swipe` | `long-press-swipe-dispatch` | User holds a key, leaves the origin key, sees the beam, and releases on a target key. | `WinFlowzKeyboardView.tryActivateLongPressSwipeFromExit` |
...
| `swipe` | `ctrl-hold-swipe-dispatch` | Gesture begins from Ctrl or from the Ctrl launch row. | `WinFlowzKeyboardView.activateCtrlHoldSwipeMode` |
| `swipe` | `target-corner-selection` | A hovered target key resolves to up/right/down/left gesture meaning. | `WinFlowzKeyboardView.longPressSwipeSelectionForTarget` |
...
- Summary: after the pointer exits the pressed origin key, WinFlowz can switch into dispatch mode, render the visual beam, track a hovered target key, and dispatch on release instead of forcing the user to toggle a separate mode.
- Entrypoints:
  - `winflowz_app/android/app/src/main/kotlin/com/winflowz_app/winflowz_app/ime/WinFlowzKeyboardView.kt`
...
- Specs / bugs:
  - `/home/claude/shipflow/shipflow_data/workflow/specs/shipflow-code-navigation-and-function-documentation-system.md`
- Decisions:
...
- Entrypoints:
  ...
  - `WinFlowzKeyboardView.canLaunchCtrlSwipeFromFrame`
- Key symbols:
  - `spaceSlideStartPx`
- Tests:
  - `See Android native validation guidance in winflowz_app/docs/technical/android-native.md`
- Specs / bugs:
...
swipe
  ...
  -> WinFlowzKeyboardView.kt / KeyboardLongPressSwipePolicy.kt
