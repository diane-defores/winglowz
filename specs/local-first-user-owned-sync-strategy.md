...
owner: "Diane"
confidence: medium
user_story: "En tant que fondatrice de VoiceFlowz, je veux que l'app fonctionne d'abord localement et puisse synchroniser via les appareils et comptes cloud des utilisateurs avec un minimum de serveurs VoiceFlowz, afin de rendre le LTD rentable sans ajouter de friction multi-appareils."
risk_level: "high"
security_impact: "yes"
docs_impact: "yes"
linked_systems:
...
supersedes: []
evidence:
  - "User report 2026-05-10: VoiceFlowz installed without Supabase configuration shows local_mode diagnostic and blank non-settings pages, proving local mode must be real product behavior, not only a backend fallback notice."
...
Draft. This spec is the first architecture and product draft for making VoiceFlowz genuinely local-first, lowering infrastructure cost for a lifetime deal, and using user-owned sync surfaces before VoiceFlowz-owned servers.
...
En tant que fondatrice de VoiceFlowz, je veux que l'app fonctionne d'abord localement et puisse synchroniser via les appareils et comptes cloud des utilisateurs avec un minimum de serveurs VoiceFlowz, afin de rendre le LTD rentable sans ajouter de friction multi-appareils.

Acteur principal: fondatrice/builder VoiceFlowz.

Acteurs secondaires: utilisateur Android, futur utilisateur desktop/mobile, utilisateur non connecte, utilisateur LTD, fournisseur de stockage cloud choisi par l'utilisateur, eventuel serveur VoiceFlowz de licence/rendezvous/relay.

Declencheurs principaux:

- L'utilisateur installe VoiceFlowz sans backend distant configure.
...
- VoiceFlowz doit vendre un LTD sans absorber une charge serveur illimitee.

Resultat observable attendu: VoiceFlowz reste utilisable et durable en local, puis propose une synchronisation optionnelle chiffree via stockage utilisateur ou P2P opportuniste. Les serveurs VoiceFlowz ne deviennent jamais la source de verite des donnees produit et ne stockent pas de contenu lisible.

# Minimal Behavior Contract

VoiceFlowz accepte les actions produit normales sans backend distant, persiste les donnees localement de facon durable, puis synchronise les changements seulement si l'utilisateur active une methode de sync. La sync produit des changements observables sur un autre appareil via un transport choisi, prioritairement un espace cloud appartenant a l'utilisateur ou une connexion P2P quand elle est possible; en cas d'echec, les changements restent locaux, visibles en etat pending ou error, et peuvent etre renvoyes plus tard sans perte. L'edge case facile a rater est le cas mobile/desktop non simultanement en ligne: le P2P direct ne suffit pas, donc la strategie doit inclure une boite aux lettres asynchrone chiffree, idealement dans le cloud de l'utilisateur, avec un relais VoiceFlowz minimal seulement comme fallback explicite.
...
- Given l'utilisateur active la sync Google Drive, Dropbox, OneDrive ou WebDAV, when l'app a obtenu l'autorisation utilisateur, then elle cree une zone d'echange app-specific et y ecrit uniquement des enveloppes chiffrees, jamais du texte lisible par VoiceFlowz ou par le provider au niveau applicatif.
...
- Given les deux appareils sont en ligne en meme temps sur des reseaux differents, when la connexion P2P reussit, then les changements peuvent etre echanges directement sans passer par un stockage central VoiceFlowz.
- Given le P2P direct echoue a cause de NAT, pare-feu ou reseau mobile, when un transport asynchrone est configure, then VoiceFlowz retombe sur la boite aux lettres cloud utilisateur ou sur un relais minimal, et l'utilisateur voit seulement un statut de sync plus lent, pas une panne bloquante.
...
- Given l'utilisateur est en LTD, when il utilise le produit sans sync cloud VoiceFlowz, then le cout variable serveur reste proche de zero hors licence, telemetry minimale et distribution.
...
- Si le token provider expire, VoiceFlowz marque le transport `needs_reauth`, garde les enveloppes locales en queue et n'efface aucune donnee acceptee.
...
- Si un relais VoiceFlowz est indisponible, la sync P2P live echoue proprement et la sync asynchrone reste disponible quand un provider utilisateur est configure.
- Si l'utilisateur perd sa cle de recovery, VoiceFlowz doit etre honnete: les donnees chiffretees hors appareil ne sont pas recuperables par VoiceFlowz.
...
La strategie doit donc separer trois choses: source de verite locale durable, transport de sync interchangeable, et serveur VoiceFlowz minimal pour licence/rendezvous/relay quand il y a une vraie necessite produit.

# Solution

Faire de VoiceFlowz une app local-first: les stores produit ecrivent d'abord dans une base locale durable, puis produisent un journal de changements chiffrable et rejouable. La synchronisation devient un pipeline optionnel et backend-agnostic qui peut utiliser des transports differents: fichier/export manuel, dossier applicatif Google Drive/Dropbox/OneDrive/WebDAV, P2P opportuniste, puis relais VoiceFlowz minimal et borne quand les autres chemins ne suffisent pas.
...
- Serveurs VoiceFlowz limites a licence, device registry minimal, rendezvous/signaling et eventuel relay rate-limited.
- Aucun contenu utilisateur lisible cote VoiceFlowz.
...
- Ajouter une strategie de pairing entre appareils: QR code, phrase de recovery ou lien court qui ne transmet jamais la cle maitre en clair a VoiceFlowz.
...
- Definir le role minimal possible des serveurs VoiceFlowz: licence/entitlement, device registry metadata, rendezvous/signaling, relay rate-limited et telemetry redacted.
...
- Construire un serveur VoiceFlowz qui stocke les donnees produit en clair.
...
- Toutes les donnees envoyees vers un provider utilisateur ou un serveur VoiceFlowz doivent etre chiffrees cote client avant transport.
- Les serveurs VoiceFlowz ne doivent pas avoir la cle permettant de lire transcriptions, clipboard, snippets, dictionnaire ou preferences sensibles.
...
- Le serveur VoiceFlowz ne peut pas lire le contenu utilisateur.
...
- TURN/relay VoiceFlowz sature ou coute trop cher.
...
- [ ] Tache 15 : Ajouter WebDAV transport avance
  ...
  - User story link : reduire dependance aux grands providers et aux serveurs VoiceFlowz.
  - Depends on : Tache 9.
...
- [ ] CA 17 : Given deux appareils en ligne sur reseaux differents, when P2P reussit, then les enveloppes sont echangees sans stockage VoiceFlowz de contenu.
...
- [ ] CA 21 : Given un utilisateur perd sa recovery key, when il demande restauration depuis cloud chiffre, then VoiceFlowz explique qu'il ne peut pas dechiffrer les donnees.
...
- Cost/ops tests before any VoiceFlowz relay:
...
- Cost risk: any VoiceFlowz relay can become expensive under LTD; must be bounded before launch.
...
7. Add any VoiceFlowz relay only with rate limits, cost model and kill switch.
...
- A proposed implementation stores plaintext sync payloads in VoiceFlowz infrastructure.
...
- Merge model: default proposal is operation log + deterministic domain-specific merge, not full CRDT dependency, because VoiceFlowz data is mostly records and settings. Revisit CRDT if real-time collaborative editing becomes a product goal.
...
- LTD positioning: default proposal is to include local-first and user-owned sync in the LTD, while reserving any heavy VoiceFlowz relay usage for bounded fair-use or future paid plans.

# Skill Run History

| Date UTC | Skill | Model | Action | Result | Next step |
|----------|-------|-------|--------|--------|-----------|
| 2026-05-10 21:41:03 UTC | sf-spec | GPT-5 Codex | Created first local-first user-owned sync strategy spec from user direction to minimize VoiceFlowz server costs and maximize local/user-cloud sync. | draft saved | `/sf-ready specs/local-first-user-owned-sync-strategy.md` |
