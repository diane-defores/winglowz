...
owner: "Diane"
user_story: "En tant que builder de la suite WinGlowz, je veux une identité client unique avec des droits séparés par produit, afin qu'un utilisateur puisse réutiliser le même compte partout sans recevoir automatiquement accès à tous les produits."
confidence: medium
risk_level: high
security_impact: "yes"
docs_impact: "yes"
...
depends_on:
  ...
  - artifact: "/home/claude/shipglowz_data/specs/master-auth-playbook.md"
...
supersedes: []
evidence:
  - "User request 2026-05-17: one account across winflows.com, WinGlowz app, and YouTube product, with access or no access per product."
  - "docs/explorations/2026-05-16-unified-suite-auth.md found no binding decision requiring separate auth per product."
  - "docs/explorations/2026-05-16-unified-suite-auth.md found VoiceFlowz historical task 'Configure Clerk for auth (shared with WinGlowz)'. User later clarified VoiceFlowz / VoiceFlows is the old name of the WinGlowz app, not a separate product."
  - "docs/DECISIONS.md targets Firebase Auth + Firestore as first hosted adapter for the WinGlowz Android app, not as a suite-wide identity decision."
  - "shipglowz_data/technical/architecture.md requires user ownership from backend auth context, not client-provided ids."
  - "firestore.rules currently scopes WinGlowz app data under users/{uid} and denies cross-user access."
  ...
  - "Canonical decision documented 2026-05-17 in /home/claude/shipglowz_data/projects/winglowz/docs/technical/suite-authentication.md: Clerk central identity, Firebase Android bridge, server-owned entitlements."
  - "Canonical support runbook added 2026-05-21 in the main WinGlowz docs and linked from the app docs."
  - "Task 10 smoke-readiness log added 2026-05-21 in shipglowz_data/workflow/TEST_LOG.md; final proof remains blocked until deployed Firebase/Convex/Firestore evidence exists."
  - "Production endpoint preflight 2026-05-22: winglowz-app and www.winglowz.com are reachable, but deployed Formation bridge endpoints `/api/bridge/firebase` and `/api/bridge/sync` return 404, so the real suite-auth smoke is blocked until deploy."
  - "Production env verification 2026-05-22: both Vercel deployments are Ready, but `winglowz` lacks Clerk/suite-auth runtime env and `winglowz-app` has no production env variables."
  ...
  - "Android device auth debug 2026-05-23: email account creation failed with Firebase `CONFIGURATION_NOT_FOUND`; CLI evidence shows `identitytoolkit.googleapis.com` is enabled on `winglowz-dev` but not on prod project `winglowz`."
...
Unified WinGlowz Suite Authentication

# Status

Ready for `/sf-start` after the 2026-05-19 readiness gate. The product direction and provider gate are explicit: Clerk is the long-term suite identity provider, Firebase Auth remains the WinGlowz Android app adapter for now, and a server-owned bridge maps Firebase users to `global_user_id`. The first proof pair is WinGlowz Formation plus the WinGlowz Android app. The bridge writes a server-owned Firestore `suiteAccess/{uid}` mirror for `winglowz_app`, app product stores require suite entitlement before using Firestore, Formation Polar grant/refund/revoke paths now call an internal sync endpoint that recomputes that mirror from Convex outside the login bridge path, and the Firebase bridge rejects revoked/disabled Firebase sessions through Firebase Admin revocation checks. The WinGlowz Formation repository is available at `/home/claude/winglowz`, and active app docs now distinguish legacy direct app-stack Clerk from Clerk as suite identity provider.

# User Story

En tant que builder de la suite WinGlowz, je veux une identité client unique avec des droits séparés par produit, afin qu'un utilisateur puisse réutiliser le même compte partout sans recevoir automatiquement accès à tous les produits.

Acteur principal: builder WinGlowz Suite.

Acteurs secondaires:

- utilisateur qui s'inscrit sur un produit puis essaie un autre produit;
- utilisateur existant sur le site WinGlowz Formation;
- utilisateur existant ou futur de l'app WinGlowz Android;
...
- Given un utilisateur a un entitlement actif `winglowz_app`, when il se connecte à l'app WinGlowz, then l'app peut utiliser son identité globale et ses données restent sous un namespace produit protégé.
...
- Provider et bridge de première tranche:
  ...
  - Firebase Auth reste l'adaptateur auth de l'app WinGlowz Android;
...
- Pas de suppression de Firebase Auth dans l'app WinGlowz avant qu'un bridge ou provider central prouvé existe.
...
- Le premier proof doit couvrir WinGlowz Formation et l'app WinGlowz Android, sinon on n'a pas prouvé la promesse suite.
...
- `docs/DECISIONS.md`: WinGlowz app cible Firebase Auth + Firestore comme premier adaptateur Android.
- `shipglowz_data/technical/architecture.md`: l'identité distante vient du backend auth context, pas du client.
- `shipglowz_data/technical/guidelines.md`: ne pas ajouter de nouveau couplage Convex/Clerk/Supabase dans l'app WinGlowz cible.
- `firestore.rules`: isolation actuelle sous `users/{uid}` pour l'app WinGlowz.
- `/home/claude/shipglowz_data/specs/master-auth-playbook.md`: standard transverse d'un propriétaire de session par runtime.
- `/home/claude/shipglowz_data/projects/VoiceFlowz/TASKS.md`: legacy tracker for the app now known as WinGlowz app; trace historique "Configure Clerk for auth (shared with WinGlowz)".
- `/home/claude/shipglowz_data/projects/winglowz/docs/technical/suite-authentication.md`: décision canonique du projet principal WinGlowz.
- `/home/claude/winglowz`: WinGlowz Formation repo; Astro 6 with Clerk, Polar, Convex, checkout and entitlement surfaces.
...
- `shipglowz_data/technical/architecture.md`: now separates direct Android app auth from suite identity bridge.
- `shipglowz_data/technical/guidelines.md`: now allows Clerk as suite identity without reviving the old app stack.
- `firestore.rules`: may need product namespace and entitlement checks if WinGlowz app begins using shared suite IDs or shared entitlement docs.
...
- WinGlowz Formation site: Clerk/Convex/Polar user and entitlement model must be audited before linking to suite identity.
...
- `/home/claude/shipglowz_data/projects/winglowz/docs/technical/suite-authentication.md`: canonical suite identity, entitlement, migration and support contract.
...
- `shipglowz_data/technical/architecture.md`: linked suite identity architecture and data boundaries.
- `shipglowz_data/technical/guidelines.md`: coding rules for global identity, entitlement checks, provider boundaries and redaction.
- Product docs for WinGlowz app, WinGlowz Formation and ReplayGlowz: login/access copy, setup env, smoke steps.
...
- User has Firebase account in WinGlowz app and Clerk account on WinGlowz Formation with same email.
...
- [ ] Tâche 1 : Aligner les docs locales WinGlowz app sur la décision canonique
  - Fichiers : `docs/DECISIONS.md`, `shipglowz_data/technical/architecture.md`, `shipglowz_data/technical/guidelines.md`, `docs/technical/suite-authentication.md`
  ...
  - Depends on : none.
  - Validate with : `rg -n "Clerk|Firebase Auth|global_user_id|suite identity|entitlement" docs/DECISIONS.md shipglowz_data/technical docs/technical/suite-authentication.md`.
...
- [ ] Tâche 2 : Écrire le contrat canonique Global Identity / Entitlements / Product Data
  - Fichiers : `/home/claude/shipglowz_data/projects/winglowz/docs/technical/suite-authentication.md`, `docs/technical/suite-authentication.md`, futurs docs API/backend du registre d'entitlements.
  ...
  - Depends on : Tâche 1.
  - Validate with : contrat relu contre WinGlowz app et WinGlowz Formation; aucun champ `user_id`, `global_user_id`, `product_id` ou entitlement client-trusted.
...
- [ ] Tâche 4 : Auditer les comptes et IDs existants avant linking
  - Fichiers : `/home/claude/shipglowz_data/projects/winglowz/docs/technical/suite-authentication.md`, `/home/claude/winglowz/convex/schema.ts`, `/home/claude/winglowz/convex/users.ts`, `/home/claude/winglowz/convex/http.ts`, `/home/claude/winglowz/src/pages/api/clerk/webhook.ts`, `/home/claude/winglowz/src/pages/api/polar/checkout.ts`
...
- [ ] Tâche 5 : Implémenter ou spécifier le registre d'entitlements serveur
  - Fichiers : `/home/claude/winglowz/convex/schema.ts`, `/home/claude/winglowz/convex/http.ts`, `/home/claude/winglowz/convex/polar.ts`, `/home/claude/winglowz/convex/users.ts`, `/home/claude/shipglowz_data/projects/winglowz/docs/technical/suite-authentication.md`, docs API/backend du registre.
...
- [x] Tâche 6 : Adapter WinGlowz app au contrat suite sans casser Firebase local-first
  ...
  - Action : exposer l'identité suite ou mapping bridge quand disponible, afficher accès `winglowz_app` selon entitlement, conserver fallback local explicite, et garder données app sous namespace protégé.
  - User story link : l'app WinGlowz participe au compte unique sans perdre sa sécurité Android/Firebase.
  ...
  - Progress 2026-05-21 : Firestore rules exigent maintenant un miroir serveur `suiteAccess/{uid}` avec `products.winglowz_app.active == true` avant les lectures/écritures sous `users/{uid}`; les providers de stores Flutter restent locaux tant que `suiteIdentityProvider` ne voit pas d'entitlement `winglowz_app`.

- [ ] Tâche 7 : Adapter WinGlowz Formation checkout/account au contrat suite
  - Fichiers : `/home/claude/winglowz/src/pages/api/polar/checkout.ts`, `/home/claude/winglowz/src/pages/api/polar/webhook.ts`, `/home/claude/winglowz/src/pages/api/clerk/webhook.ts`, `/home/claude/winglowz/convex/http.ts`, `/home/claude/winglowz/convex/users.ts`, `/home/claude/winglowz/src/utils/courseGating.ts`, `/home/claude/winglowz/shipglowz_data/technical/architecture.md`.
...
- [ ] Tâche 8 : Adapter ReplayGlowz sans confondre YouTube OAuth et suite identity
  ...
  - Notes : ne jamais utiliser un refresh token YouTube comme identité suite; cette tâche peut attendre la preuve WinGlowz Formation + app Android.

- [ ] Tâche 9 : Écrire le flow UX/support "compte reconnu, accès non actif"
  - Fichiers : produits concernés; `/home/claude/shipglowz_data/projects/winglowz/docs/technical/suite-authentication.md`; support runbook.
...
- [ ] Tâche 10 : Vérifier la première tranche inter-produit avant ship
  - Fichiers : `docs/VERIFICATION.md`, `shipglowz_data/workflow/TEST_LOG.md`, docs produit concernées.
  - Action : consigner smoke réel WinGlowz Formation + WinGlowz Android app: signup/signin, entitlement allow, entitlement deny, session restore, sign-out, webhook grant/revoke, backend data deny.
  ...
  - Progress 2026-05-21 : smoke-readiness log ajouté dans `shipglowz_data/workflow/TEST_LOG.md`; le smoke est défini comme preuve minimale end-to-end WinGlowz Formation + WinGlowz app, et la checklist sépare preuves locales, environnement déployé et actions manuelles Diane.
  - Progress 2026-05-22 : preflight production non-secret effectué; `winglowz-app.vercel.app` et `www.winglowz.com` répondent, mais les endpoints Formation `/api/bridge/firebase` et `/api/bridge/sync` retournent 404. Le smoke réel reste bloqué par déploiement.
  - Progress 2026-05-22 : après push, les endpoints Formation répondent 500 et les logs Vercel montrent `Publishable key is missing` côté Clerk. Les variables Vercel production requises pour `winglowz` et `winglowz-app` ne sont pas configurées.
  - Progress 2026-05-22 : fix local ajouté dans `/home/claude/winglowz` pour que `/api/bridge/*`, les webhooks et les APIs newsletter bypassent Clerk; `/api/polar/checkout` reste Clerk-authentifié. Preuve locale: le bridge répond maintenant par erreur JSON contrôlée sans secrets Clerk, et le CORS reflète seulement les origines autorisées.
...
- [ ] CA 2 : Given un agent frais lit la spec et la décision canonique WinGlowz, when il cherche le provider de première tranche, then il voit Clerk central identity + Firebase Android bridge sans option concurrente active.
...
- [ ] CA 12 : Given la première tranche est proposée au ship, when `sf-verify` relit les preuves, then WinGlowz Formation et WinGlowz Android app prouvent compte, entitlement allow/deny, backend deny et sign-out.
...
- Manual smoke:
  - create test user through WinGlowz Formation / Clerk;
  - open WinGlowz Android app with no `winglowz_app` entitlement;
  - grant `winglowz_app` entitlement via test webhook/manual admin;
  - retry WinGlowz Android app access;
  - revoke entitlement;
  - verify backend denies after revoke.
- Documentation checks:
  - `rg -n "global_user_id|product_entitlements|suite identity|entitlement|tenant|custom claims" docs shipglowz_data`;
...
- Read first:
  ...
  - `/home/claude/winglowz/AGENT.md`;
  - `/home/claude/winglowz/shipglowz_data/technical/architecture.md`;
  - `/home/claude/winglowz/shipglowz_data/technical/context-function-tree.md`;
...
- Keep the first implementation slice narrow: WinGlowz Formation plus WinGlowz Android app.
- Prefer provider-neutral domain contracts in WinGlowz app; do not make Flutter UI depend directly on a new provider SDK unless Task 1 selects that provider.
...
- Stop conditions:
  - provider verdict diverges from the canonical WinGlowz decision;
  - WinGlowz Formation repo or backend path becomes unavailable for the first proof pair;
...
- First proof pair: WinGlowz Formation + WinGlowz Android app.
...
| 2026-05-17 09:20:15 UTC | sf-docs | GPT-5 Codex | Documented the canonical suite auth decision in the main WinGlowz project and added short project pointers | Decision captured: Clerk central identity, Firebase Android bridge, server-owned entitlements; spec still needs lifecycle update for readiness | `/sf-spec unified-suite-authentication provider decision` |
| 2026-05-17 11:52:48 UTC | sf-spec | GPT-5 Codex | Updated spec from canonical WinGlowz suite auth decision | Provider gate, first proof pair and product ID canon resolved; spec moved to reviewed for readiness gate | `/sf-ready shipglowz_data/workflow/specs/unified-suite-authentication.md` |
| 2026-05-17 13:09:59 UTC | sf-docs | GPT-5 Codex | Corrected VoiceFlowz / VoiceFlows naming across suite auth docs | VoiceFlowz is now documented as legacy naming for WinGlowz app; separate `voiceflowz` product id removed from the auth spec | `/sf-ready shipglowz_data/workflow/specs/unified-suite-authentication.md` |
| 2026-05-17 13:13:31 UTC | sf-ready | GPT-5 Codex | Re-evaluated readiness after provider and VoiceFlowz corrections | Not ready: WinGlowz Formation repo/backend path is unavailable locally, active app docs still conflict with Clerk suite identity, and dependency versions need refresh | `/sf-spec unified-suite-authentication readiness fixes` |
...
| 2026-05-19 18:56:43 UTC | sf-ready | GPT-5 Codex | Re-evaluated readiness after `/home/claude/winglowz` became available and app docs were reconciled | Ready: first proof pair paths exist, Clerk legacy wording is scoped to old app stack, and dependency versions are refreshed | `/sf-start unified-suite-authentication` |
...
| 2026-05-23 08:52:14 UTC | sf-prod | GPT-5 Codex | Reran the failed Firestore deploy job after WIF/service-account setup | Still blocked: WIF authentication succeeds, but Firebase CLI still receives Service Usage IAM 403 when checking `firestore.googleapis.com` | add `roles/serviceusage.serviceUsageConsumer` on the `winglowz` project IAM for `github-actions-firebase-deploy@winglowz.iam.gserviceaccount.com`, then rerun failed jobs |
...
| 2026-05-23 11:52:06 UTC | sf-auth-debug + sf-prod | GPT-5 Codex | Retested hosted WinGlowz Clerk custom domain and Flutter web app auth readiness after Vercel DNS cleanup | Partial: `www.winglowz.com/signin` now renders Clerk sign-in in Playwright with email/password/Google controls and no console/request failures; `app.winglowz.com` still runs in local mode because Vercel `winglowz-app` is missing `FIREBASE_DEV_APP_ID` and `FIREBASE_WEB_CLIENT_ID` in production and preview | add missing `winglowz-app` Vercel env vars, redeploy Flutter web, then rerun deployed Firebase auth + suite bridge smoke |
| 2026-05-23 12:28:51 UTC | sf-prod + sf-auth-debug | GPT-5 Codex | Verified `winglowz-app` Vercel env completion, redeployed production, and ran hosted Flutter web auth smoke | Partial: production deployment `dpl_43qBcWKhhWJqzLstWDvML7CRum2j` is Ready and aliased to `app.winglowz.com`; Flutter web now initializes Firebase Auth/Firestore and shows the Firebase auth screen; email/password smoke reaches Firebase but provider is disabled, and Google web needs the official GIS rendered-button flow instead of app-owned `authenticate()` UI | enable Email/Password in Firebase Authentication, then patch Flutter web Google sign-in to use `google_sign_in_web` `renderButton` + authentication events |
| 2026-05-23 13:07:16 UTC | sf-auth-debug | GPT-5 Codex | Patched Flutter web Google Sign-In after hosted `authenticate-unsupported` repro | Fixed locally: Flutter web now renders the official GIS button, listens to `google_sign_in_web` authentication events, and exchanges the returned Google ID token with Firebase Auth; Android keeps the native `authenticate()` path | push/redeploy `winglowz-app`, then retest Google web sign-in on `app.winglowz.com` |
...
| 2026-05-23 19:33:42 UTC | sf-auth-debug | GPT-5 Codex | Read-only verification of the current WinGlowz Flutter web auth path after ReplayGlowz ClerkJS bridge discussion. | Confirmed: `winglowz_app` web currently uses Firebase Auth plus `google_sign_in_web` rendered GIS button, then maps the Firebase uid/id token through the suite identity bridge; no ClerkJS bridge exists in `web/index.html`, `auth_session_provider.dart`, or auth data code. | keep current Firebase web smoke path for WinGlowz app unless a new spec explicitly migrates Flutter web to ClerkJS |
| 2026-05-24 14:08:24 UTC | sf-auth-debug | GPT-5 Codex | Verified hosted WinGlowz Formation + Flutter web auth flows after env/DNS setup | Partial and patched locally: `app.winglowz.com` initializes Firebase, Email/Password reaches Firebase with controlled invalid-credential UX, Google GIS button renders and opens Google, and focused email/password fields expose browser autofill metadata; production `www.winglowz.com/fr/signin` is blocked by CSP for Clerk custom-domain scripts and `/api/bridge/firebase` still returns `firebase_admin_not_configured`. Local Formation patch now allows Clerk custom-domain CSP and reads bridge secrets through server runtime env. | push/redeploy `/home/claude/winglowz`, run `/sf-prod winglowz`, then rerun real account suite-auth smoke |
