...
owner: "Diane"
confidence: medium
user_story: "En tant que propriétaire de WinGlowz, je veux vendre l'application Android WinGlowz en Lifetime Deal Early Bird avec une page dédiée, des mentions cohérentes sur le site et un plan de lancement vérifiable, afin de commencer les ventes sans promesse produit, paiement ou accès non prouvée."
risk_level: "high"
security_impact: "yes"
docs_impact: "yes"
linked_systems:
...
supersedes: []
evidence:
  - "User request 2026-06-11: commencer à vendre l'application Android WinGlowz en Lifetime Deal Early Bird."
  ...
  - "winglowz_site/src/content/products/{fr,en}/winglowz.md currently presents WinGlowz as Windows training, not the Android voice-first app."
...
next_step: "/103-sf-verify WinGlowz Founder checkout and entitlement flow after Lemon Squeezy test-mode variants are configured"
---

# Spec: WinGlowz Android Lifetime Deal Launch

## Title

WinGlowz Android Lifetime Deal Launch

## Status

Draft created on 2026-06-11 from Diane's launch request and a repository scan of the WinGlowz app business docs, editorial governance, product pages, navigation, and checkout code. Diane confirmed on 2026-06-11 that this launch must use Lemon Squeezy because Polar is not currently available. Diane also clarified that price must not be decided by instinct: cloud sync and future cloud-dependent features can create material cost and bankruptcy risk, so an explicit pricing/competitor audit is required before setting the Early Bird price. The pricing audit was created on 2026-06-12 and recommends a tiered, sustainability-first LTD ladder rather than one cheap all-in offer. A bounded design draft was created on 2026-06-12 at `/winglowz-founder` and `/fr/winglowz-founder`; on the same day it was connected to four internal WinGlowz App Founder offers through the existing Lemon Squeezy checkout route and the suite entitlement ledger. On 2026-06-18 the payment activation contract was documented for future use. The chantier remains high-risk because hosted Lemon Squeezy checkout smoke, signed webhook smoke, refund/revoke smoke, final public copy/pricing, and device activation enforcement still require verification or implementation before broad launch.

## User Story

En tant que propriétaire de WinGlowz, je veux vendre l'application Android WinGlowz en Lifetime Deal Early Bird avec une page dédiée, des mentions cohérentes sur le site et un plan de lancement vérifiable, afin de commencer les ventes sans promesse produit, paiement ou accès non prouvée.

## Minimal Behavior Contract

Quand un visiteur arrive sur les surfaces publiques WinGlowz, le site doit présenter clairement l'offre Founder Early Bird de WinGlowz App, expliquer que l'application disponible maintenant est Android mais que le Lifetime Deal couvre les plateformes présentes et futures une fois publiées, orienter les liens pertinents vers une page de vente dédiée haut de gamme, puis envoyer l'acheteur vers un checkout Lemon Squeezy configuré pour les offres `winglowz_app/focus`, `winglowz_app/power`, `winglowz_app/control`, ou `winglowz_app/command` sans réutiliser par erreur une offre SocialGlowz ou formation Windows. La page de vente doit être suffisamment belle, crédible et persuasive pour servir de surface de lancement principale: direction visuelle forte, hiérarchie nette, assets produit réels ou mockups Android soignés, argumentaire commercial complet, CTA répétés sans agressivité, et limites honnêtes. Si le checkout, le prix, l'accès après achat, ou une preuve produit manque, le site doit afficher une attente honnête ou bloquer la mise en vente plutôt que publier un CTA trompeur. L'edge case facile à rater est la confusion de marque: le site contient encore des mentions de WinGlowz comme formation Windows, alors que cette vente vise l'application productive voice-first.
...
- La première vue annonce l'offre littérale: WinGlowz Founder Access, Lifetime Deal, Early Bird, Android disponible maintenant, plateformes futures sans deadline, avec un rendu visuel premium immédiatement identifiable.
...
- Les CTA de vente utilisent un `offerId` WinGlowz App dédié parmi `winglowz_app/focus`, `winglowz_app/power`, `winglowz_app/control`, et `winglowz_app/command`, ou un lien externe explicitement choisi par Diane et documenté.
- Le produit WinGlowz dans `src/content/products/{fr,en}/winglowz.md` est réaligné avec l'application Android ou scindé pour éviter de confondre formation Windows et app Android.
...
- Si le checkout WinGlowz Android n'est pas configuré, les CTA de paiement ne doivent pas rediriger vers `socialglowz/lifetime_deal`, `/api/polar/checkout` formation, ou un produit fournisseur ambigu.
...
WinGlowz veut commencer à vendre l'application Android, mais le site et les contrats commerce ne sont pas encore alignés avec cette vente. Le contenu produit `winglowz.md` décrit actuellement une formation Windows, la homepage et le pricing parlent de plans génériques, et le checkout générique Lemon Squeezy est centré sur SocialGlowz. Polar existe dans le monorepo mais n'est pas disponible pour ce lancement et le checkout Polar actuel est lié aux cours. Une page de vente isolée risquerait donc de créer une promesse publique sans paiement fiable, sans cohérence produit, et sans plan de lancement.

## Solution

Créer un chantier de lancement en trois blocs: une page de vente Founder LTD Early Bird, un réalignement transverse des mentions et liens publics, et un plan de lancement vérifiable. L'implémentation doit d'abord établir une source de vérité d'offre et de checkout pour WinGlowz App, puis construire la page et les liens autour de cette source, avec des claims limités aux preuves disponibles.

## Scope In

- Créer une page de vente dédiée bilingue pour l'offre WinGlowz App Founder Lifetime Deal Early Bird.
...
- Définir ou intégrer des identifiants d'offre commerce dédiés à WinGlowz App, distincts de SocialGlowz et de la formation Windows.
...
- Refaire toute la marque WinGlowz ou toute la homepage au-delà des chemins nécessaires vers l'offre.
- Résoudre toutes les dettes SocialGlowz/commerce générique hors besoin WinGlowz App Founder LTD.
...
- Ne pas réutiliser `socialglowz/lifetime_deal` pour WinGlowz Android.
- Ne pas laisser `/api/commerce/checkout` tomber silencieusement sur l'offre SocialGlowz quand une page WinGlowz appelle un checkout sans `offerId`.
...
- Required scenario IDs:
  ...
  - `WFZ-LTD-003`: product catalog and detail pages route visitors to the Android offer without describing WinGlowz only as Windows training.
  - `WFZ-LTD-004`: checkout route recognizes the WinGlowz App founder offers and never defaults a WinGlowz CTA to SocialGlowz.
...
- WinGlowz App Founder LTD is a separate offer from SocialGlowz LTD and from Windows Mastery training.
...
- A buyer must not be able to click a WinGlowz Android CTA and land in a SocialGlowz or Windows training checkout by default.
...
- Commerce code may need to move from SocialGlowz-specific helpers toward multi-product offer helpers, or deliberately add a bounded WinGlowz Android path without broad abstraction drift.
...
- Lemon Squeezy is the selected provider for this launch; provider docs and env examples must add WinGlowz Android-specific keys and tests.
...
- Update `shipglowz_data/technical/platforms/lemonsqueezy.md` because Lemon Squeezy handles WinGlowz App founder offers for this launch.
...
- Product page and training page both use the name WinGlowz and compete in search/navigation.
- French and English pages diverge in legal/commercial meaning.
- A static page is cached after the offer closes.

## Implementation Tasks

- [x] Task 1: Run the WinGlowz Android LTD pricing audit.
...
- [x] Task 2: Decide and encode the WinGlowz App founder offer contract.
  - File: `winglowz_site/src/lib/commerce/offers.ts`
  - Action: Add dedicated Lemon Squeezy-backed offer ids, product id, plans, sources, provider candidates, and helper tests for WinGlowz App Founder LTD.
...
- [x] Task 3: Add provider configuration for WinGlowz App checkout.
...
- [ ] Task 9: Audit and update public links and mentions.
  ...
  - Validate with: `rg -n "WinGlowz|Lifetime|Early Bird|Android|maitrise-windows|windows-mastery|socialglowz/lifetime_deal" winglowz_site/src` plus browser proof.
...
- [x] Task 12: Add commerce tests for launch safety.
  ...
  - Action: Cover WinGlowz App offer recognition, checkout route behavior, explicit offer IDs, and absence of wrong-product CTA defaults.
...
- [ ] CA 1: Given a visitor opens the French Founder route, when the page loads, then it clearly presents WinGlowz Founder Lifetime Deal Early Bird, Android current availability, future released-platform entitlement, and activation limits before the first paid CTA.
...
- [ ] CA 3: Given a paid CTA is rendered, when its href or request payload is inspected, then it targets a WinGlowz App offer id or an explicitly approved external checkout, never SocialGlowz or Windows training.
- [ ] CA 4: Given checkout provider env vars are absent, when the checkout route is called for WinGlowz App, then it returns an honest unavailable error and does not fallback to another offer.
- [ ] CA 5: Given the product catalog renders WinGlowz, when a buyer reads it, then the catalog does not describe only Windows training while pointing to an Android app sale.
...
- Confusing WinGlowz training and WinGlowz Android app under the same product slug.
...
| 2026-06-11 19:20:34 UTC | 100-sf-spec | GPT-5 Codex | Added pricing audit as a prerequisite before setting the Early Bird price, including competitor pricing, LTD history where sourced, cloud-cost exposure, support burden, and no-bankruptcy floor. | partial | Run /204-sf-market-study WinGlowz Android LTD pricing audit. |
...
| 2026-06-11 19:13:17 UTC | 100-sf-spec | GPT-5 Codex | Integrated Diane's provider decision: WinGlowz Android LTD launch uses Lemon Squeezy for now because Polar is unavailable. Updated scope, tasks, dependencies, and open questions accordingly. | partial | Clarify price, LTD inclusion, access delivery, launch language, and Early Bird limit/deadline; then rerun readiness. |
...
| 2026-06-11 19:07:18 UTC | 100-sf-spec | GPT-5 Codex | Created the WinGlowz Android Lifetime Deal launch spec from user request plus local scan of app business docs, editorial claim governance, product pages, navigation, checkout routes, and commerce offer code. | draft | /101-sf-ready shipglowz_data/workflow/specs/winglowz-android-lifetime-deal-launch.md |
