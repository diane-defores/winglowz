...
const pages: Record<ScriptPageKey, Record<Language, ScriptInstallPageContent>> = {
	termux: {
		en: {
			...
			command: 'curl -fsSL https://winflowz.com/termux-script | sh',
			...
			terminalLines: [
				...
				'4/6 Installation des tunnels ShipFlow',
			...
			installed: ['MyNeovimTermux', 'JetBrainsMono Nerd Font', 'Starship, Zoxide, Ranger', 'Mosh, tmux, ShipFlow local tunnels', 'termux-theme with the thermux command'],
		...
		fr: {
			...
			installed: ['MyNeovimTermux', 'JetBrainsMono Nerd Font', 'Starship, Zoxide, Ranger', 'Mosh, tmux, tunnels locaux ShipFlow', 'termux-theme avec la commande thermux'],
	...
	dotfiles: {
		en: {
			...
			command: 'curl -fsSL https://winflowz.com/dotfiles-script | sh',
			...
			excluded: ['ShipFlow system setup still uses its own root installer', 'system services are not silently enabled from user mode', 'private secrets are not created by the bootstrap'],
			linksTitle: 'Useful links',
			links: [
				{ label: 'Termux mobile profile', href: '/termux' },
				{ label: 'ShipFlow installer', href: '/shipflow' },
			],
			copyLabel: 'Copy command',
			copiedLabel: 'Copied',
			rawScriptLabel: 'Open raw script',
		...
		fr: {
			...
			excluded: ['ShipFlow garde son installateur système root séparé', 'les services système ne sont pas activés silencieusement en mode utilisateur', 'les secrets privés ne sont pas créés par le bootstrap'],
			linksTitle: 'Liens utiles',
			links: [
				{ label: 'Profil mobile Termux', href: '/fr/termux' },
				{ label: 'Installateur ShipFlow', href: '/fr/shipflow' },
			],
			copyLabel: 'Copier la commande',
			copiedLabel: 'Copié',
			rawScriptLabel: 'Ouvrir le script brut',
	...
	shipflow: {
		en: {
			slug: 'shipflow',
			name: 'ShipFlow Script',
			kicker: 'Server and agent workflow setup',
			title: 'Install the operating layer around agent-built work.',
			description:
				'ShipFlow combines a server control layer with an AI work discipline: context maps, scoped tasks, verification gates, skills, tunnels, PM2, Caddy, Flox, and the commands used to run real projects without losing the thread.',
			command: 'curl -fsSL https://winflowz.com/shipflow-script | sudo sh',
			rawScriptUrl: '/shipflow-script',
			githubUrl: 'https://github.com/dianedef/ShipFlow',
			githubLabel: 'ShipFlow repository',
			accent: 'green',
			visualLabel: 'root install',
			terminalLines: [
				'Préparation de l’installation ShipFlow...',
				'Téléchargement de ShipFlow...',
				'Mode root confirmé',
				'Configuration ShipFlow multi-utilisateur',
			],
			fitTitle: 'Best for',
			fit: ['servers that run active projects', 'AI-assisted product work', 'fresh-agent handoffs', 'PM2, Caddy, Flox and tunnel operations'],
			installedTitle: 'Installed',
			installed: ['ShipFlow CLI commands: shipflow, sf, s', 'Claude/Codex skill symlinks', 'server tooling and wrappers', 'local project tracking data', 'terminal TUI commands when available'],
			...
			linksTitle: 'Useful links',
			links: [
				{ label: 'ShipFlow public docs', href: 'https://github.com/dianedef/ShipFlow' },
				{ label: 'Dotfiles installer', href: '/dotfiles' },
			],
			copyLabel: 'Copy command',
			copiedLabel: 'Copied',
			rawScriptLabel: 'Open raw script',
			installNote: 'Run from a sudo-capable account. The bootstrap prepares ~/shipflow for that user, then delegates system setup to install.sh as root.',
		},
		fr: {
			slug: 'shipflow',
			name: 'Script ShipFlow',
			kicker: 'Setup serveur et workflow agents',
			title: 'Installe la couche d’opération autour du travail produit par agents.',
			description:
				'ShipFlow combine une couche de contrôle serveur et une discipline de travail IA: cartes de contexte, tâches cadrées, gates de vérification, skills, tunnels, PM2, Caddy, Flox et commandes pour faire tourner de vrais projets sans perdre le fil.',
			command: 'curl -fsSL https://winflowz.com/shipflow-script | sudo sh',
			rawScriptUrl: '/shipflow-script',
			githubUrl: 'https://github.com/dianedef/ShipFlow',
			githubLabel: 'Dépôt ShipFlow',
			accent: 'green',
			...
			installedTitle: 'Installé',
			installed: ['commandes ShipFlow: shipflow, sf, s', 'symlinks de skills Claude/Codex', 'outillage serveur et wrappers', 'tracking local des projets', 'commandes TUI terminal quand disponibles'],
			...
			linksTitle: 'Liens utiles',
			links: [
				{ label: 'Docs publiques ShipFlow', href: 'https://github.com/dianedef/ShipFlow' },
				{ label: 'Installateur dotfiles', href: '/fr/dotfiles' },
			],
			copyLabel: 'Copier la commande',
			copiedLabel: 'Copié',
			rawScriptLabel: 'Ouvrir le script brut',
			installNote: 'À lancer depuis un compte avec sudo. Le bootstrap prépare ~/shipflow pour cet utilisateur, puis délègue l’installation système à install.sh en root.',
		},
	},
}
