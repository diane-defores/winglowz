...
function parseSnapshot(value: unknown): ReplayGlowzEntitlementSnapshot | null {
  ...
  if (!ALLOWED_REASON_CODES.has(reasonCode as ReplayGlowzEntitlementReasonCode)) {
    return null;
  }
...
export const POST: APIRoute = async ({ request }) => {
  ...
  if (!endpointSecret || !convexBridgeSecret) {
    return jsonResponse(
      { status: "unavailable", error: "entitlement_bridge_secret_not_configured" },
      503
    );
  }
  ...
  try {
    const rawSnapshot = await convex.query(
      "bridge:getReplayGlowzEntitlementSnapshotByClerkId" as never,
      {
        clerkId: clerkUserId,
