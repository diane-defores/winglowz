...
evidence:
  - "WinFlowzKeyboardView.KeyFrame currently stores one RectF used for both drawing and hit testing."
  - "WinFlowzKeyboardView.hitTest returns the first key frame whose rect contains the touch point."
...
depends_on: []
supersedes: []
next_step: "shipflow_data/workflow/specs/keyboard-stable-grid-touch-geometry.md"
---
...
Diane asked whether WinFlowz can keep its current keyboard design, spacing, theme, and customization while making the tactile layer behave more like a gapless grid, because fast typing sometimes misses keys.

## Context Read

- `android/app/src/main/kotlin/com/winflowz_app/winflowz_app/ime/WinFlowzKeyboardView.kt` - source of touch handling, hit testing, row drawing, key drawing, scroll rows, and debug overlay.
- `android/app/src/main/kotlin/com/winflowz_app/winflowz_app/ime/KeyboardLayoutModels.kt` - source of key and row layout data.
- `android/app/src/main/kotlin/com/winflowz_app/winflowz_app/ime/KeyboardThemeModels.kt` - source of theme spacing settings such as key gaps and width scale.
...
- Created spec: `shipflow_data/workflow/specs/keyboard-stable-grid-touch-geometry.md`
