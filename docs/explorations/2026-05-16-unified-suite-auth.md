...
evidence:
  ...
  - "/home/claude/.claude/projects/-home-claude-winflowz/751f382a-c2fc-42ea-bf75-b4e0c48dd1c5"
depends_on:
...
- `docs/DECISIONS.md` - Current reviewed decision is backend-agnostic data
  ...
  WinFlowz app. It does not decide suite-wide identity.
- `shipflow_data/technical/architecture.md` - User ownership comes from backend
  auth context, not client-provided IDs. This remains valid under either shared
  or product-local auth.
- `shipflow_data/technical/guidelines.md` - Firebase Auth + Firestore is current
  WinFlowz app direction; Convex/Clerk/Supabase are legacy for this app, not a
  workspace-wide prohibition.
- `/home/claude/shipflow_data/specs/master-auth-playbook.md` - Cross-project
...
- Pros: Good compromise for existing mixed stack: WinFlowz site can stay
  Clerk/Convex for now, WinFlowz app can stay Firebase while a central identity
...
|----------|--------------|--------|--------|-----------|
| 2026-05-16 00:00:00 UTC | Shared account across product suite | Searched local WinFlowz docs, ShipFlow auth playbook, project trackers, historical WinFlowz session notes, and current identity-provider docs | No binding decision found against shared suite identity; evidence favors shared identity with separate entitlements | `/sf-spec unified-suite-authentication` |
