...
# Comment rester productif quand on travaille seul sur Windows

Le travail solo n’est pas juste une version allégée du travail d’équipe. C’est un autre problème. La même personne doit porter le projet, le contexte et le rythme.

## Pourquoi le travail solo se complique

Quand personne n’est en face du processus, les points faibles restent cachés plus longtemps :

- on se souvient au lieu de capturer
...
Un bon setup solo doit réduire le nombre de décisions nécessaires pour avancer.

Cela veut souvent dire :

1. Une entrée simple pour les nouvelles idées.
2. Un moyen clair de transformer les notes en actions.
3. Une façon de retrouver le travail inachevé sans friction.
4. Moins de dépendance à la mémoire, plus de dépendance au processus.

## Pourquoi Windows compte
...
Si vous travaillez seul, commencez par ce qui se répète tous les jours :

- capturer le même type d’entrée au même endroit
- garder une source de vérité pour le travail actif
- supprimer les étapes qui n’existent que parce que le système est fragmenté
- rendre l’action suivante visible sans recherche
